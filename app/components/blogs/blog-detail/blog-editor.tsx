...
import { useBlogsContext } from "@/app/contexts/blogs-context";
import { useBlogState } from "@/app/hooks/blogs/use-blog-state";
import { useBlogSubmit } from "@/app/hooks/blogs/use-blog-submit";
...
export default function BlogEditor({
  ...
  const blogsCtx = useBlogsContext();
  const { blogs, setBlogs, hostId, editingBlogId: ctxEditingId } = blogsCtx;
  ...
  const combinedResetEditor = () => {
    resetEditor();
    blogsCtx.resetEditor?.();
  };

  const { handleSubmit, submitAs, isSaving, handleDelete, isDeletingId } =
    useBlogSubmit({
      hostId,
      blogs,
      setBlogs,
      editingBlogId: ctxEditingId ?? null,
      form,
      coverFile,
      tagInput,
  ...
  // Sync when parent opens editor for a specific blog id
  useEffect(() => {
    const idToEdit = blogId ?? blogsCtx.editingBlogId ?? null;
    if (idToEdit && blogs) {
      const b = blogs.find((x) => x.id === idToEdit);
      if (b) startEdit(b as any);
    }
  }, [blogId, blogsCtx.editingBlogId, blogs, startEdit]);

  if (action === "delete") {
    return (
      <DeleteBlog
        editingBlogid={blogId ?? ctxEditingId ?? undefined}
        onClose={onClose}
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              ...
              {ctxEditingId ? "Update blog" : "Publish blog"}
            </Button>
            <Button
              type="button"
              variant="outline"
