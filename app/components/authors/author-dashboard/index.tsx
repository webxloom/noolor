...
// Contexts & Hooks
import { AuthorContextProvider } from "@/app/contexts/author-context";
...
export default function AuthorDashboard({
  ...
  const bioStatus = Boolean(
    form.pen_name && form.bio && (form.location || form.phone),
  );

  return (
    <AuthorContextProvider value={profile}>
      {/* Author completion progress */}
      {canEdit && (
        <RoleCompletionCriteria
          roleId={profile?.authorId || null}
          role={role}
          bioStatus={bioStatus}
        ...
        <div className="rounded-lg border bg-background/80 flex-1 min-w-0">
          ...
          >
            ...
            <TabsContent value={activeTab} className="space-y-6">
              {activeTab === "about" ? (
                <AuthorDetail canEdit={canEdit} />
              ) : activeTab === "books" ? (
                <BooksList
                  canEdit={canEdit}
                  roleId={profile.authorId}
                  role={role}
                />
              ) : activeTab === "blogs" ? (
